<template>
  <div class="profile container " style="margin-top: 5px">
    ...
    <div class="row block-list">
      <loader v-if="!trans.data"></loader>
      <transaction-list :transListData="trans.data" v-else></transaction-list>
    </div>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  //  blocklist
  import TransactionList from 'compDom/TransactionList/TransactionList';
  //  Loader
  ...
  export default {
    ...
    async created() {
      this.getAllTransactions(this.query);
    },
    computed: {
      ...mapGetters({
        trans: "blockchain/allTransactions" 
      }),
      currentPage() {
        return (this.query.start / this.query.limit) + 1;
      },
      totalPage() {
        return this.trans.total ? Math.ceil(this.trans.total / this.query.limit) : 0;
      }
    },
    methods: {
      //
      ...
      // nextPage
      nextPage () {
        this.query.start = ((this.query.start + this.query.limit) > this.trans.total) ? this.query.start : (this.query.start + this.query.limit);
        this.getAllTransactions(this.query);
      },
      // prevPage
      prevPage () {
