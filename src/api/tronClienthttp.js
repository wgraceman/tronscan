...
const {AccountList, NumberMessage, WitnessList, AssetIssueList} = require("@tronprotocol/wallet-api/src/protocol/api/api_pb");
const {TransferContract} = require("@tronprotocol/wallet-api/src/protocol/core/Contract_pb");
const qs = require("qs");
const buildApplyForDelegate = require("@tronprotocol/wallet-api/src/utils/transaction").buildApplyForDelegate;
const longToByteArray = require("@tronprotocol/wallet-api/src/utils/bytes").longToByteArray;
...
const { getBase58CheckAddress, signTransaction, privateKeyToAddress, SHA256 } = require("@tronprotocol/wallet-api/src/utils/crypto");

const ONE_TRX = 1000000;

class TronClientHttp {
  ...
  /** v2 */
  async getBlocks(param) {
    let params = jQuery.param(param);
    let {data} = await xhr.get(`${this.url}/block?${params}`);

    return data;
  }
  async getBlockByAddress(witnessAddress) {
    let {data} = await xhr.get(`${this.url}/account/${witnessAddress}`);
   ...
   * @returns {Promise<*>}
   */
  async getBlockByNum(witnessAddress) {
    let {data} = await xhr.get(`${this.url}/account/${witnessAddress}`);
    let currentBlock = base64DecodeFromString(data);
  ...
  async getAccountList(param) {
    let params = jQuery.param(param);
    let {data} = await xhr.get(`${this.url}/account?${params}`);
   ...
   */
  async getWitnesses() {
    let {data} = await xhr.get(`${this.url}/witness`);
